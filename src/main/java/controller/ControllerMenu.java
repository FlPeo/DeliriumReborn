...
public class ControllerMenu implements EventHandler<MouseEvent> {
    ...
    public void handle(MouseEvent event) {
        if(event.getSource().equals(launcher.getMenu().getNouvellePartieJoueur())
            ...
            launcher.demarrerPartie(Integer.parseInt(launcher.getMenu().getChoixNiveau().getSelectedToggle().getUserData().toString()));
        }
        else if(event.getSource().equals(launcher.getMenu().getNouvellePartieIA())
                && event.getEventType().equals(MouseEvent.MOUSE_CLICKED) ) {
            launcher.demarrerPartieIA(Integer.parseInt(launcher.getMenu().getChoixNiveau().getSelectedToggle().getUserData().toString()));
        }
